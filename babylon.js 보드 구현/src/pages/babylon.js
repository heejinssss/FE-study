  ...
  useEffect(() => {
    ...
    for (let i = 0; i < numRows; i++) {
      for (let j = 0; j < numRows; j++) {
        ...
        // Check if the current box is within the central 25 blocks
        if (i >= halfSize - 2 && i <= halfSize + 2 && j >= halfSize - 2 && j <= halfSize + 2) {
          box.visibility = 0; // Set the visibility of the box to 0 (invisible)
          
          // Load and position the glTF model on the first block
          if (i === halfSize - 2 && j === halfSize - 2) {
            BABYLON.SceneLoader.ImportMesh('', 'public/model/', 'scene.gltf', scene, (newMeshes) => {
              const model = newMeshes[0];
              model.position = new BABYLON.Vector3(box.position.x, box.position.y + tileSize / 2 + 0.1, box.position.z);
            });
          }
        }
      }
